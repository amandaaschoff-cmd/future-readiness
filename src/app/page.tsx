...
export default function FutureReadinessAssessment() {
  ...
  const isCurrentBlockComplete = useMemo(() => {
    ...
    return qs.every((_, j) => {
      ...
      return (answers as any)[key] !== undefined && (answers as any)[key] !== null;
    });
  ...
  useEffect(() => {
    ...
    for (const d of DOMAINS) {
      console.assert(
        Array.isArray((QUESTIONS as any)[d]) && (QUESTIONS as any)[d].length === 3,
  ...
  const reset = () => {
    setAnswers({} as Record<string, number>);
    setShowGate(false);
  };
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        <style jsx global>{`
          /* Fallback for Tailwind line-clamp plugin on Vercel */
          .line-clamp-2 { display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
          .group:hover .line-clamp-2, .group:focus-within .line-clamp-2 { -webkit-line-clamp: unset; display: block; overflow: visible; }
        `}</style>
        {/* Load Figtree font */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_.9fr] gap-10 lg:gap-16 items-center">
            ...
            <Card className="bg-white/90 text-slate-900 rounded-2xl shadow-lg overflow-hidden lg:mt-10">
              <CardContent className="p-0">
                <img
                  src="https://via.placeholder.com/800x450.png?text=Benchmark+preview"
